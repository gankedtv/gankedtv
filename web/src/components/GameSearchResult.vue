...
<template>
  <li
    ...
    class="flex cursor-pointer items-center gap-3 px-3 py-2 transition-colors duration-150"
    :class="selected ? 'bg-accent-bg text-accent' : 'hover:bg-surface-high'"
    @mousedown.prevent="$emit('select')"
  >
    <span class="text-[10px] font-bold uppercase tracking-[0.06em] text-accent">
      {{ tag }}
    </span>
    <span class="text-xs text-text-primary">{{ name }}</span>
  </li>
</template>

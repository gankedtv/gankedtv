...
function toggleFollow() {
  ...
  fireToast(following.value ? `Following @${user.value.username}` : `Unfollowed @${user.value.username}`)
}
    ...
    <div class="clip-layout">
      ...
      <div>
        ...
        <div
          ...
          style="font-family: var(--font-mono); font-size: 11px; color: var(--color-text-muted); letter-spacing: 0.08em; text-transform: uppercase;"
        >
        ...
        <!-- Video Player -->
        <div
          style="aspect-ratio: 16/9; background: #000; border-radius: var(--radius-md); overflow: hidden; border: 1px solid var(--color-border); position: relative;"
        >
          <!-- Thumbnail -->
          <img
            :src="clip.art"
          ...
          >
            <div class="flex items-center gap-2">
              <span
                ...
                style="background: var(--color-brand); font-family: var(--font-mono); font-size: 10px; letter-spacing: 0.12em; text-transform: uppercase; font-weight: 600;"
              >{{ game.tag }}</span>
              <span style="font-family: var(--font-mono); font-size: 11px; color: rgba(255,255,255,0.55); letter-spacing: 0.06em;">{{ clip.id }}</span>
            </div>
            <button
          ...
          >
            <div
              ...
              style="width: 64px; height: 64px; background: rgba(0,0,0,0.55); border: 2px solid rgba(255,255,255,0.25); backdrop-filter: blur(4px);"
            >
              <!-- Play icon -->
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <!-- Play/Pause mini -->
              <button class="text-white hover:text-text-secondary transition-colors" @click="togglePlay">
              ...
              <!-- Time -->
              <span style="font-family: var(--font-mono); font-size: 11px; color: rgba(255,255,255,0.7);">
              ...
              <span
                ...
                style="font-family: var(--font-mono); font-size: 10px; background: rgba(255,255,255,0.12); color: rgba(255,255,255,0.7); letter-spacing: 0.05em;"
              >1x</span>
        ...
        <div class="mt-5">
          <h1
            style="font-family: var(--font-heading); font-weight: 700; font-size: 34px; text-transform: uppercase; letter-spacing: 0.01em; line-height: 1.05; color: var(--color-text-primary);"
          >{{ clip.title }}</h1>
          ...
          <div class="flex flex-wrap items-center gap-3 mt-4">
              ...
              <div>
                <div class="flex items-center gap-1.5">
                  <router-link
                    ...
                    style="font-family: var(--font-mono); font-size: 13px; font-weight: 600; color: var(--color-neon);"
                ...
                <div
                  style="font-family: var(--font-mono); font-size: 10px; color: var(--color-text-muted); letter-spacing: 0.04em;"
                >
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                @click="toggleLike"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" :fill="liked ? '#fff' : 'currentColor'">
              ...
              <button
                class="flex items-center gap-1.5 px-3 py-1.5 rounded transition-all duration-150"
                style="background: var(--color-surface-raised); border: 1px solid var(--color-border); color: var(--color-text-secondary); font-family: var(--font-mono); font-size: 12px;"
              >
              ...
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11A2.99 2.99 0 0 0 18 8a3 3 0 0 0 0-6 3 3 0 0 0 0 6c.34 0 .67-.06.98-.14L12 11.99A2.99 2.99 0 0 0 9 11a3 3 0 0 0 0 6 2.99 2.99 0 0 0 2.98-2.77l7.02 4.12c-.31.08-.63.13-.97.13a3 3 0 0 0 3 3 3 3 0 0 0 0-6z"/>
                </svg>
                <span>Share</span>
              </button>
            </div>
        ...
        >
          ...
          >
            <span
              style="font-family: var(--font-mono); font-size: 10px; color: var(--color-text-muted); letter-spacing: 0.08em; text-transform: uppercase;"
            >{{ stat.label }}</span>
            <span
              style="font-family: var(--font-heading); font-size: 20px; font-weight: 700; color: var(--color-text-primary); line-height: 1.2;"
            >{{ stat.value }}</span>
          </div>
        </div>
        ...
        >
          <div
            class="mb-2"
            style="font-family: var(--font-mono); font-size: 10px; color: var(--color-text-muted); letter-spacing: 0.1em; text-transform: uppercase;"
          >Description</div>
          <p class="text-sm mb-3" style="color: var(--color-text-secondary); line-height: 1.6;">
            {{ clip.title }}. Ranked match, prime time lobby. Everything was on the line. Check the timestamp at peak action — the rotation read was insane.
          </p>
          <div class="flex flex-wrap gap-2">
            <span
              ...
              :key="tag"
              class="px-2 py-0.5 rounded"
              style="font-family: var(--font-mono); font-size: 11px; background: var(--color-surface-overlay); border: 1px solid var(--color-border); color: var(--color-text-muted); letter-spacing: 0.05em;"
            >#{{ tag.replace('#', '') }}</span>
          </div>
        </div>
        ...
        <div class="mt-8">
          ...
          <div class="flex items-center gap-3 mb-5 section-title-bar">
            <span
              style="font-family: var(--font-heading); font-size: 18px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; color: var(--color-text-primary);"
            >Chat</span>
            <span
              class="px-2 py-0.5 rounded-full"
              style="font-family: var(--font-mono); font-size: 11px; background: var(--color-surface-raised); border: 1px solid var(--color-border); color: var(--color-text-muted);"
          ...
          <div class="flex items-start gap-3 mb-6">
            ...
            <div class="flex-1 flex gap-2">
              <input
                ...
                style="background: var(--color-surface-raised); border: 1px solid var(--color-border); color: var(--color-text-primary); font-family: var(--font-body);"
                @focus="($event.target as HTMLElement).style.borderColor = 'var(--color-border-strong)'"
          ...
          <div class="flex flex-col gap-5">
            ...
            >
              <UserAvatar :user="Object.keys(USERS).find(k => USERS[k].username === c.user) ?? c.user" :size="32" />
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <span style="font-family: var(--font-mono); font-size: 12px; font-weight: 600; color: var(--color-neon);">@{{ c.user }}</span>
                  <span style="font-family: var(--font-mono); font-size: 11px; color: var(--color-text-muted);">{{ c.time }}</span>
                </div>
                <p class="text-sm mb-2" style="color: var(--color-text-secondary); line-height: 1.5;">{{ c.text }}</p>
                <div class="flex items-center gap-3">
                  <button
      ...
      <div class="flex flex-col gap-6">
        ...
        >
          <div class="px-4 py-3" style="border-bottom: 1px solid var(--color-border);">
            <div class="flex items-center gap-2">
              <span
                class="rounded-full"
                style="width: 7px; height: 7px; background: var(--color-neon); display: inline-block; animation: pulse 2s ease-in-out infinite;"
              />
              <span style="font-family: var(--font-mono); font-size: 10px; color: var(--color-text-muted); letter-spacing: 0.08em; text-transform: uppercase;">
          ...
          <div v-if="relatedClips[0]" class="p-3 flex gap-3 items-start">
            ...
            <div class="flex-1 min-w-0">
              <p
                ...
                style="color: var(--color-text-primary); line-height: 1.35; font-family: var(--font-body);"
              >{{ relatedClips[0].title }}</p>
              <router-link
        ...
        <div>
          <div
            class="flex items-center gap-3 mb-4 section-title-bar"
          >
            <span
              style="font-family: var(--font-mono); font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--color-text-primary);"
          ...
          <div class="flex flex-col gap-1">
            <router-link
              ...
              @mouseenter="($event.currentTarget as HTMLElement).style.background = 'var(--color-surface-raised)'"
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <p
                  class="text-sm font-semibold mb-1 line-clamp-2"
                  style="color: var(--color-text-primary); line-height: 1.35; font-family: var(--font-body); font-size: 13px;"
                >{{ related.title }}</p>
                <span
                  style="font-family: var(--font-mono); font-size: 11px; color: var(--color-neon);"

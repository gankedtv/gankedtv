...
5. **After all worktrees are created:**
   ...
   - If `OPEN_ITERM=true`, invoke the `worktree-open` skill with the issue list. The skill opens one iTerm tab per worktree, each split into four side-by-side panes (`make web`, `make server`, `claude`, `make up`).
